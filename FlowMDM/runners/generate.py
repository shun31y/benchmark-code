...
def load_humanml_training_stats():
...
def feats_to_xyz(sample, dataset, batch_size=1):
    if dataset == 'humanml': # for HumanML3D
        n_joints = 22
        mean, std = load_humanml_training_stats()
...
def main():
    ...
    if not is_using_data: 
        ...
        with open(args.instructions_file, 'r') as f:
            ...
            if args.use_history_text:
                assert "history_text" in instructions, "Instructions file must contain 'history_text' when --use_history_text is enabled"
    ...
    if is_using_data:
        ...
        if "history_text" in instructions:
            model_kwargs['y']['history_text'] = instructions["history_text"]
        with open(os.path.join(animation_out_path, "prompted_texts.json"), "w") as f:
...
def load_dataset(args, split):
    ...
    data = get_dataset_loader(name=args.dataset,
                              ...
                              num_workers=1,
                              use_history_text=args.use_history_text)
