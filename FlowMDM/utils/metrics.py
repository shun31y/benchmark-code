...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file, gt_stats=None):
...
def generate_plot_PJ(path):
    ...
    transition_metrics = results_json.get("transition", {})
    if not isinstance(transition_metrics, dict):
        return None

    values = transition_metrics.get("PeakJerk")
    if not values:
        return None
    ...
    if isinstance(values, dict):
        ...
        series_by_name = {"transition": values}
    series_by_name = {
        name: series
        for name, series in (
    ...
    transition_length = max(len(series) for series in series_by_name.values())
    output_path = os.path.join(output_dir, name_path + "_transition_PJ" + ".png")
