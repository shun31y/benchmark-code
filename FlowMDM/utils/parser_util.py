...
def add_history_text_options(parser):
    group = parser.add_argument_group('history_text')
    group.add_argument("--use_history_text", action='store_true',
                       help="Use cumulative history text to mix CLIP text embeddings.")
    group.add_argument("--history_current_weight", default=1.0, type=_float_0_1,
                       help="Current-caption weight for history text conditioning. Must be in [0, 1].")
...
def add_training_options(parser):
    ...
    group.add_argument("--train_platform_type", default='WandbPlatform', choices=['NoPlatform', 'TensorboardPlatform', "WandbPlatform"], type=str,
    ...
    group.add_argument("--eval_num_samples", default=100, type=int,
...
def add_sampling_options(parser):
    ...
    add_history_text_options(parser)
...
def add_evaluation_options(parser):
    ...
    group.add_argument("--eval_target", default='auto', choices=['auto', 'motion_only', 'motion_and_transition'], type=str,
                       help="Evaluation target selection. auto uses motion_only for anaphora, otherwise motion and transition.")
    group.add_argument("--eval_file", default='', type=str,
                       help="Optional path to an evaluation JSON file.")
    group.add_argument("--target_segment_from_id", action='store_true',
                       help="Evaluate only the segment encoded by __ana_segXX_ in each sample id.")
    group.add_argument("--eval_batch_size", default=32, type=int,
                       help="Batch size for metric computation. Keep 32 for HumanML3D R-precision compatibility.")
    group.add_argument("--generation_batch_size", default=128, type=int,
                       help="Batch size used only when precomputing/generated evaluation motions.")
    add_history_text_options(parser)
