...
def babel_collate(batch):
    ...
    for ii in range(bs):
        ...
        if 'history_text' in batch:
            adapted_batch[-1]['history_text'] = batch['history_text'][ii]
...
def collate(batch):
    ...
    if 'history_text' in notnone_batches[0]:
        cond['y'].update({'history_text': [b['history_text'] for b in notnone_batches]})
...
def t2m_collate(batch):
    ...
    if len(batch[0]) >= 10 and batch[0][9] is not None:
        for i, b in enumerate(adapted_batch):
            b['history_text'] = batch[i][9]
    return collate(adapted_batch)
