...
def get_dataset(name, num_frames, split='train', load_mode='train', **kwargs):
    ...
    if name in ["humanml"]:
        normalized_load_mode = load_mode
        if load_mode != "gt_anaphora_fullgen" and "gt" in load_mode:
            normalized_load_mode = "gt"
        dataset = DATA(
            normalized_load_mode,
            split=split,
            pose_rep=pose_rep,
            num_frames=num_frames,
            use_history_text=kwargs.get('use_history_text', False),
