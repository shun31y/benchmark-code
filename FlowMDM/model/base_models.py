...
from utils.history_text import validate_text_embeddings_shape
...
class TextConditionalModel(nn.Module):
    ...
    def encode_text(self, raw_text):
        ...
        max_text_len = 24 if self.dataset in ['humanml'] else None  # Specific hardcoding for humanml dataset
        if max_text_len is not None:
            default_context_length = 77
            context_length = max_text_len + 2 # start_token + text tokens + end_token
    ...
    def compute_embedding(self, x, timesteps, y):
        ...
        text_embeddings_cache_key = None
        if "text_embeddings" in y:
            text_embeddings_cache_key = (id(y["text_embeddings"]), tuple(y["text_embeddings"].shape))

        if not force_mask:
            if 'text' == self.cond_mode:
                primitive = frozenset(y[key]) if not multitext_mode else frozenset((frozenset(txts) for txts in y[key]))
                if text_embeddings_cache_key is not None:
                    primitive = (primitive, text_embeddings_cache_key)
            else:
                raise ValueError
        ...
        if not multitext_mode: # --> single text training (e.g. HumanML3D dataset) / inference
            if "text_embeddings" in y:
                ...
                validate_text_embeddings_shape(enc_text, (bs, self.clip_dim))
            else:
            ...
            if conditions_mask.ndim != 3:
                raise ValueError(f"conditions_mask must have shape [I, T, N], got {tuple(conditions_mask.shape)}")
            if conditions_mask.shape[1] != nframes or conditions_mask.shape[2] != bs:
                raise ValueError(
                    f"conditions_mask shape mismatch: expected [I, {nframes}, {bs}], got {tuple(conditions_mask.shape)}"
                )
            if "text_embeddings" in y: # preloaded for fast training / eval
                enc_text = y["text_embeddings"]
                validate_text_embeddings_shape(enc_text, (conditions_mask.shape[0], bs, self.clip_dim))
            else:
                ...
                validate_text_embeddings_shape(enc_text, (conditions_mask.shape[0], bs, self.clip_dim), "encoded_text")

            I, N, d = enc_text.shape
            enc_text = enc_text.reshape(-1, enc_text.shape[-1]) # [I*N, d]
            embedded_text = self.embed_text(self.mask_cond(enc_text, force_mask=force_mask)).reshape(I, N, self.latent_dim) # [I, N, d]
